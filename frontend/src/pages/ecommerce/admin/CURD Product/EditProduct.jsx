...
export default function EditProduct() {
  ...
  const [existingImages, setExistingImages] = useState([]); // Simpan gambar yang sudah ada
  ...
  const fetchProduct = async () => {
    try {
      ...
      if (result.success && result.data) {
        const productImages = Array.isArray(result.data.images)
          ? result.data.images
          : [];
        ...
        setFormData({
          ...
          gender: normalizeGenderList(result.data.gender),
          ...
          featured_image: result.data.featured_image || productImages[0] || "",
        ...
        // Set image preview dan existing images
    ...
    try {
      ...
      Object.keys(formData).forEach((key) => {
        if (formData[key] !== null && formData[key] !== undefined) {
          if ((key === "tags" || key === "gender") && Array.isArray(formData[key])) {
      ...
      // Tambahkan existing images dan file upload baru
